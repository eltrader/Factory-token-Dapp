...
import FCTTokenFactoryContract from '../contracts/FCTTokenFactory'
import FCTTokenFactory from '../factory/fcttokenfactory'
import TokenContract from '../contracts/FCTToken'
import TokenFactory from '../factory/tokenfactory'
import { Web3Instance } from '../services/web3Instance'
import { isConnected } from '../util/fct'
...
export default class Content extends Component {
    ...
    async load() {
        this.setState({
            ...
            this.setState({
                ...
                const FCTFactoryContract = await FCTTokenFactoryContract(this._web3Instance.getProvider())
                this.FCTFactory = new FCTTokenFactory(FCTFactoryContract)

                this.setState({
                    paidTokenAddress: (await this.FCTFactory.getPaidTokenAddress(this.state.account)),
                    paidTokenDecimals: (await this.FCTFactory.getPaidTokenDecimals(this.state.account))
                }, async () => {
                    const Token = await TokenContract(this.state.paidTokenAddress, this._web3Instance.getWeb3(), this.state.account)
                    this.FCTTokenFactory = new TokenFactory(Token)

                    this.setState({
                        fctAllowance: (await this.FCTTokenFactory._allowance(
                            this.state.account,
    ...
    async setPrice() {
        if (this.state.tokenType.includes('Basic')
            ...
            this.setState({
                price: this.state.payment.includes('FCT')
                    ? (await this.FCTFactory.getDeployPriceFCT(this.state.account))
                    : (await this.FCTFactory.getDeployPriceBNB(this.state.account)),
            })
        }
    }
    ...
    getApproveOrCreateButton() {
        if (this.state.payment.includes('FCT')
            && parseFloat(this.state.fctAllowance) < parseFloat(this.state.price))
    ...
    approve() {
        this.setState({
            ...
            await this.FCTTokenFactory._approve(
                this.FCTFactory._address(),
            ...
            this.setState({
                fctAllowance: (await this.FCTTokenFactory._allowance(
                    this.state.account,
                    this.FCTFactory._address(),
                    this.FCTFactory.getPaidTokenDecimals()
                ))
            }, () => {
                this.setState({
                    loading: false
    ...
    async deploy() {
        ...
        if (code === 0)
            return this.FCTFactory.deployFree(
            ...
            if (this.state.payment.includes('FCT')) {
                return this.FCTFactory.deployPaidFCT(
                    this.state.tokenName,
                    this.state.tokenSymbol,
                    this.state.tokenDecimals,
                    this.state.tokenSupply,
                ...
                return this.FCTFactory.deployPaidBNB(
                    this.state.tokenName,
                    this.state.tokenSymbol,
                    this.state.tokenDecimals,
                    this.state.tokenSupply,
